        ...
        <v-layout justify-center row wrap>
          <v-flex xs2 style="margin:0px 20px 20px 0px" v-for="(item,index) in items" :key="index">
            <v-card @click="selectFunction(item)" :class="[item.state?'active1':'']">
              ...
              <div v-if="item.state" class="active3">
                <v-icon color="white" x-large>check_circle</v-icon>
              </div>
              <v-img :src="getCategoryData[index].imageUrl" height="150px" contain></v-img>
  ...
  methods: {
    ...
    selectFunction(item) {
      ...
      this.items.forEach(ele => {
        if (ele.state == true) {
          temp += 1;
          if (ele.categoryName == item.categoryName) sel = true;
        }
      });
      if (temp < 4 || sel) item.state = !item.state;
      else alert("Maximum no of preferences selected!!");
    },
    savePreferences() {
